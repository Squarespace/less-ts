...
import { REPLACE, STRING } from '../../src/plugins/string';
...
const resolve = () => {
  ...
  const result = REPLACE.invoke(env, [quoted('abc 123'), quoted('([a-z]+) ([0-9]+)'), quoted('$2 $1')]);
  return { env, result };
};
...
test('replace: the experimental warning is raised on both sides', () => {
  ...
  REPLACE.invoke(fixed, [quoted('abc 123'), quoted('([a-z]+) ([0-9]+)'), quoted('$2 $1')]);
...
test('replace: fixed levels keep dollar group references literal', () => {
  ...
  const legacyRes = REPLACE.invoke(legacy, [quoted('abc 123'), quoted('([a-z]+) ([0-9]+)'), quoted('$1 is $2')]);
  expect(legacy.ctx.render(legacyRes as Quoted)).toBe('"abc is 123"');

  const env = new LessCompiler({ compatLevel: 2 }).context().newEnv();
  const result = REPLACE.invoke(env, [quoted('abc 123'), quoted('([a-z]+) ([0-9]+)'), quoted('$1 is $2')]);
...
test('replace: renders the pinned warning comment before the rule', () => {
  ...
  const value = REPLACE.invoke(env, [quoted('abc 123'), quoted('([a-z]+) ([0-9]+)'), quoted('$2 $1')]);
...
test('replace: validation requires three args', () => {
  const env = context().newEnv();
  const [ok, errors] = REPLACE.validate(env, [quoted('abc 123')]);
  expect(ok).toBe(false);
  expect(errors.length).toBe(1);

  const [ok2] = REPLACE.validate(env, [quoted('abc 123'), new Anonymous('([a-z]+)'), quoted('x')]);
